...
def test_check_no_momentum_tickers_returns_empty():
    ...
    with patch("tailwind_options.sources._helpers.yf.download", return_value=download_result):
...
def test_check_error_on_ticker_continues():
    ...
    calls = _make_calls_df([{"contractSymbol": "NVDA_OK", "strike": 165.0, "ask": 3.0}])
...
def test_filter_by_momentum_uses_cache():
    ...
    with patch("tailwind_options.sources._helpers.yf.download", return_value=download_result) as mock_dl:
        scanner = TrendCallScanner(universe=["TSLA"])
...
def test_scan_calls_no_expirations_returns_empty():
    ...
    with patch("yfinance.Ticker", return_value=mock_ticker):
        scanner = TrendCallScanner(universe=["TSLA"])
        signals = scanner._scan_options("TSLA", 90.0, 150.0)
    assert signals == []
