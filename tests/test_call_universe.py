...
from tailwind_options.call_universe import _fetch_from_yf, get_tickers
...
def test_get_tickers_s3_old_format_refetches():
    ...
    with patch("tailwind_options.call_universe.yf.Ticker", return_value=_mock_ticker(["AAPL"])):
        result = get_tickers(s3=mock_s3, bucket="my-bucket")
...
def test_get_tickers_s3_miss_fetches_from_yf():
    ...
    with patch("tailwind_options.call_universe.yf.Ticker", return_value=_mock_ticker(["AAPL", "TSLA"])):
        result = get_tickers(s3=mock_s3, bucket="my-bucket")
...
def test_fetch_from_yf_returns_sorted():
    with patch("tailwind_options.call_universe.yf.Ticker", return_value=_mock_ticker(["TSLA", "AAPL", "NVDA"])):
        tickers, _ = _fetch_from_yf()
    assert tickers == sorted(tickers)


def test_fetch_from_yf_etf_error_returns_empty():
    with patch("tailwind_options.call_universe.yf.Ticker", side_effect=Exception("network error")):
        tickers, names = _fetch_from_yf()
    assert tickers == []
    assert names == {}
...
def test_fetch_from_yf_skips_invalid_symbols():
    with patch("tailwind_options.call_universe.yf.Ticker", return_value=_mock_ticker(["-", "nan", "TSLA"])):
        tickers, _ = _fetch_from_yf()
