...
export default function ReportPage({ config }: { config: ScanConfig }) {
  ...
  const [selectedThemeId, setSessionTheme] = useSessionTheme(base)
  ...
  const heroes = report ? report.heroes as unknown as HeroMap : undefined
  ...
  function scrollToHero(heroRowId: string) {
    ...
    if (report) {
      ...
      const heroTicker = heroes?.[term]?.ticker
      if (heroTicker) {
  ...
  }, [report, heroes, heroIdPrefix, base])

  const selectedEntry = manifest.find(r => r.id === selectedId)
  const isMorning     = selectedEntry ? getScanLabel(selectedEntry.generated_at) === 'Morning Data' : true
  ...
  const heroCards = report
    ? HERO_TERMS.map(term => {
        ...
        return {
          ...
          emptyLabel: HERO_CARD[term].emptyLabel,
