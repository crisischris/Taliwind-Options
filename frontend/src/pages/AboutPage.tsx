...
function ThemeColumn({ title, page }: { title: string; page: 'puts' | 'calls' }) {
  ...
  const [selectedId, setSelectedId] = useState(themes.find(t => !t.stub)?.id ?? themes[0].id)
      ...
      <CollapsibleContent>
        <div className="flex flex-wrap gap-2 mb-8">
          {themes.map(t => (
            <button
              key={t.id}
              onClick={() => !t.stub && setSelectedId(t.id)}
              className={cn(
            ...
            >
              {t.name}
              {t.stub && (
                <Badge variant="outline" className="text-[9px] py-0 px-1.5 h-4 leading-none">Soon</Badge>
              )}
            </button>
          ))}
        </div>
  ...
  if (theme.stub) {
    return (
      <div className="text-muted-foreground text-sm space-y-2">
        <p className="italic">{theme.tagline}</p>
        <p>{theme.thesis}</p>
        <p className="mt-4 font-medium text-foreground">Scanner coming soon.</p>
      </div>
    )
  }
