...
export default function HeroCard({ option, movePct, moveLabel, label, icon, emptyLabel, heroRowId, onScrollTo }: Props) {
  if (!option) return (
    <Card className="h-full border-2 border-dashed border-border/50 bg-card/40">
      <CardContent className="p-5 h-full flex flex-col items-center justify-center text-center gap-3 min-h-[12rem]">
        <span className="text-3xl opacity-50">{icon}</span>
        <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">{label}</p>
        <p className="text-sm text-muted-foreground max-w-[18ch]">{emptyLabel}</p>
      </CardContent>
    </Card>
  )
